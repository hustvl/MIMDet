...
class ConvStem(nn.Module):
    ...
    def __init__(self,
        ...
        for idx in range(depth):
            ...
            if idx == depth - 1:
                stage_list.append(
                    nn.Conv2d(output_dim, embed_dim, kernel_size=1))
...
class MIMDetEncoder(nn.Module):
    def __init__(
        ...
        self.checkpointing = checkpointing
        self.patch_embed = ConvStem(img_size,
                                    patch_size,
                                    in_chans,
                                    embed_dim,
        ...
        dpr = [x.item() for x in torch.linspace(0, dpr, depth)
        ...
        for idx, shape in enumerate(feature_shapes):
            ...
            for img_idx, (h, w) in enumerate(image_sizes):
                masks_per_feature_level[img_idx, :int(
                    np.ceil(float(h) / self.patch_embed.patch_size[0])
                ), :int(np.ceil(float(w) /
                                self.patch_embed.patch_size[1])), ] = 0
    ...
    def forward(self, imgs, sample_ratio):
        ...
        x = outputs[-1]
        H, W = x.shape[-2:]
        masks = self.mask_out_padding([x.shape], imgs.image_sizes,
                                      imgs.tensor.device)
        x = x.flatten(2).transpose(1, 2)
        pos_embed = interpolate_pos_embed_online(self.pos_embed,
...
class MIMDetDecoder(nn.Module):
        ...
        if pretrained:
            ...
            for k, v in checkpoint_model.items():
                if 'encoder' in k:
                    ...
                    new_checkpoint_model[k] = v
            interpolate_pos_embed(self, new_checkpoint_model,
                                  'decoder_pos_embed')
            print(self.load_state_dict(new_checkpoint_model, strict=False))
    ...
    def forward(self, x, ids_restore, new_size):
        ...
        pos_embed = interpolate_pos_embed_online(self.decoder_pos_embed,
...
class MIMDetBackbone(Backbone):
    ...
    def output_shape(self):
        return {
            name: ShapeSpec(channels=self._out_feature_channels[i],
